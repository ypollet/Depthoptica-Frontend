...
const DEFAULT_IMG: StackImage = { name: "RBINS Logo", label: "RBINS", thumbnail: "", image: "https://www.naturalsciences.be/bundles/8c62adb1e0fbef009ef7c06c69a991890012e203/img/logos/logo.svg", size: { height: 200, width: 200 } }
...
export const useImagesStore = defineStore('images', {
  state: () => ({
    objectPath: "",
    image: "",
    images: new Map<string, StackImage>(),
    zoom: -1,
    offset: { x: 0, y: 0 }
  }),
  getters: {
    selectedImage: (state) => state.images.get(state.image) ?? DEFAULT_IMG
  },
  actions: {
    setPath(path: string) {
      this.$reset()
