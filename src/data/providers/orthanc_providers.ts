...
export class OrthancProvider implements DataProvider {
    ...
    async computeProfile(objectPath: string, imageName: string, start: Coordinates, end: Coordinates, edgeThreshold : string | undefined = undefined, smooth : boolean = true): Promise<AxiosResponse>{
        const path = this.server + '/depthoptica/' + imageName + "/profile?x1=" + start.x + "&y1=" + start.y + "&x2=" + end.x + "&y2=" + end.y + "&threshold=" + edgeThreshold + "&smooth=" + smooth
        return axios.get(path)
    };
